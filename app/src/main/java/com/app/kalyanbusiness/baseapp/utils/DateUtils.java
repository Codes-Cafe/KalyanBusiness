...
public class DateUtils {
    ...
    public static String getFormattedTimeToShow(String dateTime) {
        try {
            ...
            String formattedDate = DateUtils.getFormattedTime(dateTime.split("T")[1]);//DateUtils.getLocalDateStringFromUTC(dateTime).split("T")[1]);
            Trace.endSection();
    ...
    public static int getDaysRemaining(String expiry) {
        ...
        int days = 0;
        try {
            days = (int) DateUtils.getDateDiff(Calendar.getInstance().getTime(), DateUtils.getDateFromString(expiry, DATE_TIME_FORMAT)
                    , TimeUnit.DAYS);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static int getDaysListed(String expiry) {
        ...
        int days = 0;
        try {
            days = (int) DateUtils.getDateDiff(DateUtils.getDateFromString(expiry, DATE_TIME_FORMAT), Calendar.getInstance().getTime()
                    , TimeUnit.DAYS);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//          System.out.println(TimeUnit.MILLISECONDS.toDays(now.getTime() - past.getTime()) + " days ago");
    ...
    public static String getTime(String dateString) {
        Date date = getDateFromString(dateString, DATE_TIME_FORMAT);
        return getTime(date);
    }
...
*/
    ...
    public static boolean isToday(String creationDate) {
        ...
        return getDateDiff(getDateFromString(creationDate, DATE_TIME_FORMAT), new Date(), TimeUnit.HOURS) < 24;
    }
    ...
    public static String getFormattedDateTimeToShow(String dateTime) {
        ...
        if (dateParts.length > 0) {
            try {
                ...
                if (!TextUtils.isStringEmpty(time))
